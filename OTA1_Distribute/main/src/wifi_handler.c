...
void wifi_init_sta()
{
    ...
    memcpy((uint8_t *)&wifi_config.sta.ssid, WIFI_SSID, strlen((char *)WIFI_SSID));
    memcpy((uint8_t *)&wifi_config.sta.password, WIFI_PASS, strlen((char *)WIFI_PASS));
    ...
    {
        ESP_LOGI(TAG_WIFI, "Failed to connect to WIFI_SSID:%s, PASS:%s",
                 WIFI_SSID, WIFI_PASS);
    }
    else
    {
